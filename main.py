# This program creates a CRUD desktop app with SQLite database.
...
class Product:
    ...
    # Create database and product table if is not already created
    def sql_connection():
        try:
    ...
    def __init__(self, window):
        ...
        # Button add product
        button = ttk.Button(frame, text='Save product', command=self.add_product).grid(row=3, columnspan=2, sticky=W + E)
        ...
        # Filling the rows
        self.get_products()
    def run_query(self, query, parameters = ()): #Realizo una funcion para establecer conexion a la base de datos
